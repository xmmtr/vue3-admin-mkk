...
整个系统分为三个模块，如图2-1所示，分别是后台系统管理，小程序用户端管理、订单购物车管理

​	系统运行计算机环境大致如下：Windows 7、 Windows 8、Windows 10、Firefox、 Chrome。

​	开发系统之前，必须考虑系统的安全性、稳定性和易维护性，操作的简单化。并且在简单化的同时要做到功能最大化。开发体统前考虑到客户需求、系统功能和性能要求以及我自身的习惯等因素。

麦鲲鲲管理信息系统，在功能上实现管理员基本信息管理、商品管理、员工工资管理、员工培训和业绩管理等。该系统的可行性分析包括以下几个方面的内容。

**2.1.1**技术可行性

商品销售管理系统是将销售管理信息化，实现了进货、退货、消费、结账等新的管理方式。我们设计的这个系统采用了 SQL Server进行数据库方面的开发，而SQLServer数据库能够处理大量的数据，还保持数据的完整行并提供了许多高级管理功能。它的灵活性、安全性和易用性为数据库编程提供了良好的条件。

**2.1.2** **经济可行性**

随着经济的快速发展，建立一个信息化的管理模式的经济代价不再是一个很大的负担了。而且，信息化以后所带来的经济效应也是可观的，是现在所要实现现代化的一个主要的目标。
...
​												**本系统总体架构图如图2-1所示**
![image](https://github.com/user-attachments/assets/dd8e93bb-ac43-4816-b355-7ffd4ccddc02)
...
关于一些商品详细信息操作：多条件分页查询操作、添加操作、更新操作、删除操作、上传商品图片操作、计算选中的金额、分析操作、显示操作。

描述：对商品详细信息表和订单表进行增删改查。

#### 2.2.1订单模块

关于一些订单详细信息操作：多条件分页查询操作、添加操作、更新操作、删除操作、计算选中的金额、分析操作、显示操作。

描述：对订单表进行增删改查。

#### 2.2.1用户模块

关于一些用户详细信息操作：多条件分页查询操作、添加操作、更新操作、删除操作、上传用户头像操作、购物车操作、计算选中的金额、分析操作、显示操作。
...
商品详细信息模块对数据表操作可用如表2-2的方式表述(R代表读,W代表写)。
...
用户模块对数据表操作可用如表2-3方式表述(R代表读,W代表写)。

**表2-3用户模块**

|                  | user | Role | Order | Commodity | Commoditytype |
| ---------------- | ---- | ---- | ----- | --------- | ------------- |
| 用户模块         | R/W  | R    | R/W   | R         | R             |
| 查询             | R    | R    | R     | R         | R             |
| 添加商品到购物车 | W    | R    | W     | R/W       | R/W           |
| 修改昵称         | R/W  |      |       |           |               |
| 修改头像         | R/W  |      |       |           |               |

 

订单详细信息模块对数据表操作可用如表2-4的方式表述(R代表读,W代表写)。

**表2-4订单详细信息模块**

|                  | Orders | Order_Commodity | user |
| ---------------- | ------ | --------------- | ---- |
| 订单详细信息模块 | R/W    | R/W             | R/W  |
| 查询             | R      | R               | R    |
| 添加             | W      | W               | R/W  |
| 修改             | R/W    | R/W             | R/W  |
| 删除             | W      | W               | W    |
...
## 2.4   模块间信息流
![image](https://github.com/user-attachments/assets/558ce17f-b064-4534-b4a2-7247aac3f0ac)
![image](https://github.com/user-attachments/assets/7d6b3efd-4457-428f-a84a-19edcf008190)
图2-5 管理系统信息流图

##   2.5   实体库关系图

![image](https://github.com/user-attachments/assets/cc1029fe-5834-4837-91c0-c752143ba1b8)
图2-6点餐系统管理及用户页面E-R模型

### 2.6   数据字典

表2-7商品表：commodity

| 字段名         | 数据类型 | 字段大小 | 是否为空 | 说明     |
| -------------- | -------- | -------- | -------- | -------- |
| Id             | 数字     | 整型     | 否       | 商品编号 |
| commodityname  | 字符串   | 255      | 否       | 商品姓名 |
| commodityintro | 字符串   | 255      | 否       | 商品分类 |
| material       | 字符串   | 30       | 是       | 商品原料 |
| Flavor         | 字符串   | 10       | 是       | 商品口味 |
| Weight         | 字符串   | 50       | 是       | 商品分量 |
| Commodityimage | 字符串   | 255      | 否       | 商品图片 |
| commoditymoney | 浮点型   | 0,0      | 否       | 商品单价 |
| commodityType  | 字符串   | 10       | 否       | 商品类型 |

 

表2-8订单表：commodityType

| 字段名        | 数据类型 | 字段大小 | 是否为空 | 说明     |
| ------------- | -------- | -------- | -------- | -------- |
| Id            | 数字     | 整型     | 否       | 编号     |
| commodityType | 文本     | 255      | 否       | 商品类别 |

 

表2-9订单表：order_commodity

| 字段名            | 数据类型 | 字段大小 | 是否为空 | 说明       |
| ----------------- | -------- | -------- | -------- | ---------- |
| Id                | 数字     | 整型     | 否       | 商品编号   |
| orderidenti       | 字符串   | 255      | 否       | 商品识别码 |
| Commodity_name    | 字符串   | 255      | 否       | 商品名称   |
| Commodity_num     | 数字     | 整型     | 否       | 商品数量   |
| Commodities_money | *浮点型* | 0,0      | 否       | 商品金额   |
| Commodity_image   | 字符串   | 255      | 否       | 商品图片   |
...
| ----------- | -------- | -------- | -------- | ---------- |
| Id          | 数字     | 整型     | 否       | 用户编号   |
| createuser  | 字符串   | 255      | 否       | 用户名     |
| phonenumber | 字符串   | 255      | 否       | 手机号码   |
| ordermoney  | 数字     | 整型     | 否       | 订单金额   |
| ordertime   | *浮点型* | 0,0      | 否       | 下单时间   |
| orderidenti | 字符串   | 255      | 否       | 订单识别码 |
...
| ----------- | -------- | -------- | -------- | -------- |
| Id          | 数字     | 整型     | 否       | 用户ID   |
| username    | 字符串   | 255      | 否       | 用户名   |
| password    | 字符串   | 255      | 否       | 密码     |
| nickname    | 字符串   | 255      | 否       | 昵称     |
| phonenumber | 字符串   | 12       | 否       | 手机号码 |
| role        | 字符串   | 10       | 否       | 用户权限 |
| userimage   | 字符串   | 255      | 否       | 用户头像 |
